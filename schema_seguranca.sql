...
-- ----------------------------------------------------------------------------
-- TABELA: admins (dono/operador da plataforma Fluxo — nível de acesso
-- "Negócio", acima de profissional e independente do mecanismo de suporte
-- interno via profissionais.is_admin. Vê e edita todos os profissionais e,
-- através deles, todos os clientes finais. Tabela separada de propósito:
-- não é um "profissional com flag", é outro nível de acesso inteiro, sem
-- cota de clientes, sem assinatura, sem cobrança.
-- ----------------------------------------------------------------------------
CREATE TABLE admins (
    id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    nome            TEXT NOT NULL,
...
-- ============================================================================
-- Toda policy abaixo tem uma cláusula extra:
--   OR current_setting('app.is_admin', true)::boolean IS TRUE
-- Isso permite que um admin autenticado (tabela `admins`, não `profissionais`)
-- enxergue e edite dados de QUALQUER profissional/cliente — é assim que o
-- nível de acesso "Negócio" consegue trocar de contexto pra ver como
-- planejador ou cliente específico, sem precisar da senha de ninguém.
--
